...
function ledger(): CredentialLedger {
  return {
    ...
    listForReview: vi.fn(),
    listMine: vi.fn(),
    readPrivateDetails: vi.fn(async () => ({ score: 92 })),
    verify: vi.fn(),
    createDisclosure: vi.fn(),
    evaluateDisclosure: vi.fn(),
    consumeDisclosure: vi.fn(),
    revokeDisclosure: vi.fn(),
    listMyDisclosures: vi.fn(),
    submitAppeal: vi.fn(),
    reviewAppeal: vi.fn(),
    readAppeal: vi.fn(),
...
describe('authentication routes', () => {
  ...
  it('rejects wrong credentials, missing CSRF and a mismatched role', async () => {
    ...
    const denied = await app.inject({
      ...
      url: '/api/v1/auth/login',
      headers: { origin },
      payload: { username: 'issuer', password: 'wrong-password' },
    });
    ...
    const issuerPrivateRead = await app.inject({
      method: 'GET',
    ...
    const missingCsrf = await app.inject({
      ...
      headers: { cookie: issuerCookie, origin },
      payload: {},
    });
    expect(missingCsrf.json()).toMatchObject({ code: 'CSRF_INVALID' });
    const batchWithoutCsrf = await app.inject({
      method: 'POST',
      url: '/api/v1/credentials/imports',
      headers: { cookie: issuerCookie, origin },
      payload: {},
    });
    expect(batchWithoutCsrf.json()).toMatchObject({ code: 'CSRF_INVALID' });
    ...
    const studentPrivateRead = await app.inject({
      method: 'GET',
      url: '/api/v1/credentials/cred:2026:api01/private-details',
      headers: { cookie: studentCookie },
    });
    expect(studentPrivateRead.json()).toMatchObject({ score: 92 });
    const disclosureWithoutCsrf = await app.inject({
      method: 'POST',
    ...
    const issuerDisclosure = await app.inject({
      method: 'POST',
      url: '/api/v1/credentials/cred:2026:api01/disclosures',
    ...
    const wrongBatchRole = await app.inject({
      method: 'POST',
      url: '/api/v1/credentials/imports',
      headers: { cookie: studentCookie, origin, 'x-csrf-token': studentLogin.json().csrfToken },
      payload: {},
    });
    expect(wrongBatchRole.json()).toMatchObject({ code: 'ROLE_FORBIDDEN' });
    await app.close();
  });
});
