...
describe('POST /api/v1/verify', () => {
  ...
  it('rejects unsigned unknown claims instead of stripping them before verification', async () => {
    const keys = makeKeys();
    const record = activeRecord();
    const app = buildApp({ ledger: makeLedger(record), issuerKeys: keys });
    const credential = exportCredential(record, privateDetails, ['courseName'], keys);
    const injected = { ...credential, assertedHonors: true };

    const response = await postVerify(app, JSON.stringify(injected));
