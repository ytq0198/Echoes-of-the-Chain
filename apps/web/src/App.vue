...
const credentialDecisionForm = ref({ reason: '原始成绩材料与当前凭证承诺不一致，请签发方核对后重新提交。', salt: 'CHAIN_GRADE_DECISION_2026' });
...
async function decideCredential(action: 'reject' | 'revoke'): Promise<void> {
  ...
  credentialDecisionState.value = 'loading'; credentialDecisionMessage.value = '';
  try {
    reviewRecord.value = await requestJson<PublicCredentialRecord>(`/api/v1/credentials/${encodeURIComponent(reviewRecord.value.credentialId)}/${action}`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(credentialDecisionForm.value) });
    credentialDecisionState.value = 'success';
    credentialDecisionMessage.value = action === 'reject' ? '草稿已驳回，理由明文仅保存于组织私有集合。' : '凭证已撤销，公开状态与私有理由承诺均已更新。';
    void loadReviewList();
  } catch (error) { credentialDecisionState.value = 'error'; credentialDecisionMessage.value = error instanceof Error ? error.message : '状态操作失败'; }
}
  ...
  <main>
    ...
    <template v-else>
      ...
      <section v-else-if="currentView === 'reviewer'" class="workspace shell">
        ...
        <article v-if="reviewRecord" class="review-sheet"><div class="sheet-head"><div><span class="status" :class="reviewRecord.status.toLowerCase()">{{ reviewRecord.status }}</span><h3>{{ reviewRecord.credentialId }}</h3></div><span>v{{ reviewRecord.version }}</span></div><dl class="record-grid"><div><dt>签发组织</dt><dd>{{ reviewRecord.issuerMspId }}</dd></div><div><dt>Schema</dt><dd>{{ reviewRecord.schemaVersion }}</dd></div><div><dt>学生匿名标识</dt><dd class="mono">{{ shortHash(reviewRecord.subjectHash) }}</dd></div><div><dt>课程标识</dt><dd class="mono">{{ shortHash(reviewRecord.courseHash) }}</dd></div><div class="wide"><dt>详情哈希承诺</dt><dd class="mono">{{ reviewRecord.detailHash }}</dd></div><div class="wide"><dt>提交者身份哈希</dt><dd class="mono">{{ reviewRecord.submittedByIdentityHash }}</dd></div><div v-if="reviewRecord.reasonHash" class="wide"><dt>决定理由哈希</dt><dd class="mono">{{ reviewRecord.reasonHash }}</dd></div></dl><div class="review-actions"><p>批准会激活待复核草稿；驳回或撤销会公开状态与理由哈希，但理由明文只进入组织私有集合。</p><button v-if="reviewRecord.status === 'PENDING_REVIEW'" class="button primary" :disabled="reviewerState === 'loading'" @click="approveCredential">批准并激活</button></div><form v-if="reviewRecord.status === 'PENDING_REVIEW' || reviewRecord.status === 'ACTIVE'" class="decision-form" @submit.prevent><div class="operation-heading"><div><p class="eyebrow">PRIVATE DECISION</p><h4>{{ reviewRecord.status === 'ACTIVE' ? '撤销有效凭证' : '驳回待复核草稿' }}</h4></div><span>理由不上公共账本</span></div><div class="field"><label for="decision-reason">处理理由</label><textarea id="decision-reason" v-model="credentialDecisionForm.reason" minlength="10" maxlength="2000" required></textarea></div><div class="field"><label for="decision-salt">理由隐私盐值</label><input id="decision-salt" v-model="credentialDecisionForm.salt" minlength="16" required /></div><button v-if="reviewRecord.status === 'PENDING_REVIEW'" type="button" class="button danger" :disabled="credentialDecisionState === 'loading'" @click="decideCredential('reject')">驳回草稿</button><button v-else type="button" class="button danger" :disabled="credentialDecisionState === 'loading'" @click="decideCredential('revoke')">撤销凭证</button><p v-if="credentialDecisionMessage" class="notice" :class="credentialDecisionState">{{ credentialDecisionMessage }}</p></form></article>
