...
const profile = profileIndex === -1 ? 'formal' : process.argv[profileIndex + 1];
if (!['formal', 'smoke'].includes(profile)) throw new Error('Profile must be formal or smoke');
const expected = profile === 'formal'
  ? { warmupSeconds: 10, sampleSeconds: 30, repeats: 3, concurrencies: [1, 5, 10, 20], cells: 84 }
  : { warmupSeconds: 0, sampleSeconds: 1, repeats: 1, concurrencies: [1], cells: 7 };
if (manifest.warmupSeconds !== expected.warmupSeconds || manifest.sampleSeconds !== expected.sampleSeconds || manifest.repeats !== expected.repeats) {
  throw new Error(`Latest evidence is not the ${profile} ${expected.warmupSeconds}/${expected.sampleSeconds}/${expected.repeats} profile`);
}
if (JSON.stringify(manifest.concurrencies) !== JSON.stringify(expected.concurrencies)) throw new Error(`${profile} concurrency matrix is incomplete`);
if (manifest.schedule.length !== expected.cells) throw new Error(`Expected ${expected.cells} cells, found ${manifest.schedule.length}`);
...
console.log(profile === 'formal'
  ? `Validated ${expected.cells} formal cells, controls, faults and ledger audit for ${runId}`
  : `Validated ${expected.cells} smoke cells and controls for ${runId}`);
