...
the evidence is incomplete or an invariant failed. The command finishes by enforcing the 84-cell
matrix, control probes, fault recovery, ledger audit, privacy scan and evidence checksums.

For a fast engineering check, `smoke` uses zero warm-up, one-second sampling, concurrency one and one repeat. Its validator checks the seven smoke cells and control probes without requiring the formal fault and ledger-audit artifacts. Environment
