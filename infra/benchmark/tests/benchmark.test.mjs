...
test('smoke and formal runs invoke their matching evidence validators', async () => {
  const source = await readFile(join(projectRoot, 'infra/benchmark/benchmark.sh'), 'utf8');
  assert.match(source, /validate-evidence\.mjs" --profile smoke/);
  assert.match(source, /validate-evidence\.mjs" --profile formal/);
});
