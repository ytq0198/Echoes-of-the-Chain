...
2. W3C VC 实现指南将选择性披露分为原子凭证、选择性披露签名、哈希值等路径。课程版可以先采用可审计的原子声明或加盐 Merkle 承诺，竞赛增强版再引入 BBS+/匿名凭证。
...
| `anoncreds/anoncreds-rs` | 活跃参考实现 | 零知识谓词证明、撤销状态、匿名凭证 | 密码学集成复杂，适合作为竞赛增强而非首个里程碑 |
...
- 课程基础版：Fabric 私有数据集合 + 链上哈希/状态 + 加盐 Merkle 字段承诺 + 限时能力链接。
- 竞赛增强版：W3C VC 2.0 + BBS+ 选择性披露；在资源允许时增加“GPA/排名达到阈值但不公开精确值”的零知识谓词证明。

优势：紧扣原题；演示链路直观；隐私、安全和多组织治理均可量化测试；课程版与竞赛版可渐进交付。
...
建议选择 A 为主线，将 C 中的“成绩版本与申诉证据”作为管理端增强，将 B 的“跨机构核验/学分互认”保留为竞赛后续模块。这样可以形成清晰的三级路线：

1. MVP：成绩录入、复核、签发、查询、修订/撤销、公开验证。
2. 课程完整版：学生钱包、最小披露、二维码、授权过期、审计看板、完整测试和美观 UI。
3. 竞赛增强版：标准 VC、不可链接选择性披露、阈值证明、跨机构互认、Caliper 与安全攻击实验。
