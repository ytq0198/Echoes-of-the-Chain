...
- `typora-latex-theme/chaingrade-latex.css`：本项目入口与兼容层；视觉规则仍由上游主题负责，只处理手写章节编号和本地封面资源。
- `typora-latex-theme/assets/`：原仓库封面使用的浙江大学校名与校徽 SVG 的本地副本。
...
- 封面结构、正文、标题、表格和打印版式以 `typora-latex-theme` v0.3.3 为主，不再使用另行设计的 ChainGrade 视觉系统。
- 封面沿用上游浙江大学模板的校名、校徽、标题与信息表结构；仅替换为本项目资料，并使用本地 SVG 防止 PDF 导出时网络图片缺失。
- 一级正文标题使用原报告已有的“1、2、3……”编号，兼容层关闭上游自动编号，避免目录和正文出现双重编号。
- 摘要/目录结束后由第一章标题触发打印分页，不再插入空分页元素，因此不会产生目录后的整页空白。
- 兼容层把上游 9.5pt 正文小幅调整为 10.5pt，并用 `@font-face` 按 Windows 字体的 PostScript 名 `LMRoman10-*`、`LMMono10-Regular` 绑定 Latin Modern；这避免了 Typora/Chromium 无法识别字体族别名时退回 Times New Roman。封面英文项目名也使用同一绑定。
- `#write` 使用 `border-box`，使 Typora 屏幕预览和 PDF 打印的正文宽度同为约 17cm；目录只显示章级标题。
- 普通截图最大高度为 18.5cm，超长浏览器截图会整体缩放，而不会被拆到两张 A4 页面；封面校名与校徽不受这一限制。
- 封面填写三名成员及学号：魏子安 `3240101782`、强璞 `3240102045`、阳震 `3240105586`；没有虚构授课教师或上课时间。
- LaTeX 风格版只改变呈现方式；120/120 项目测试、7/7 基准工具测试、覆盖率、账本证据、VC 验收、性能数据、故障记录和局限性均沿用原报告。
