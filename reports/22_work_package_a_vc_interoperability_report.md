...
- [ ] 新功能无需演示账号硬编码和私钥入库
- [ ] 原有测试全部通过，新增不少于 18 个有效测试（当前新增：shared 32 + api 64）
...
- [ ] 学生导出前字段预览和隐私说明可见
- [ ] 未选择字段时不能生成空凭证
- [ ] 公开验证结果分别展示「签名、链上状态、详情锚定」（非单一绿灯）
- [ ] 文件解析失败、网络失败、已撤销、已取代、篡改 各有不同说明

## 6. 负向验证结果表

| 场景 | 预期 | 实际 | 截图 |
| --- | --- | --- | --- |
| 篡改 courseName / score / detailHash | 签名段失败，结论 invalid | | ![篡改](assets/iteration-14-vc/negative-tampered.png) |
...
| 上传非 JSON / 重复键 / 缺字段 | 400 解析失败 | | ![解析失败](assets/iteration-14-vc/negative-parse-error.png) |
| 上传超限 | 413 | | ![超限](assets/iteration-14-vc/negative-oversize.png) |
