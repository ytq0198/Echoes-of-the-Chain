...
PPTX 导出后以 1600×900 分辨率重新渲染 26 页，自动越界检查通过。根据复核意见，又将结论页的生成插图作为全稿背景母题：封面使用完整原图，普通内容页采用低透明度背景，结论页采用较高透明度权重并移除重复前景图。第二轮逐页渲染确认背景没有降低文字、代码、截图和数据的可读性，标题与对象仍无截断。
